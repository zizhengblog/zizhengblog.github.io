...
hash2就是签名数据，编译阶段xcode会用Mac上存储的私钥对APP进行签名，这个值就是用Mac私钥对hash值的签名数据。那么如何验证呢？我们使用该工程中用到的P12文件对hash值做一次签名，看得到的结果与hash2值是否一致来验证我们的猜想。


<img src="/images/encrypted/sign12.png" alt="img">

<span style="color:red">关于hash2暂时没有验证成功，可能理解有误</span>
...
5、资源验签：hashembedded.mobileprovision包含的证书公钥、CodeResources文件内每个文件的hash、hash2，进行签名验证，如果验证成功证明该资源文件的<span style="color:red">签名数据</span>没有被篡改过。然后对该资源文件进行同样的sha1摘要算法，如果得到的值与hash相同证明该<span style="color:red">资源文件</span>没有被修改或替换过（应用完整性问题解决）。
