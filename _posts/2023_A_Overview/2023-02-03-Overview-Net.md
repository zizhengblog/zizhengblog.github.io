...
同网段      
arp广播，收到广播的主机查看是否是自己的ip地址，如果不是丢弃，如果是将ip地址返回，交换机会记录mac地址             
...
封装成帧：帧首部、帧尾部、帧开始符、帧结束符        
透明传输：对帧开始符、帧结束符进行转义        
差错检验：帧尾部的FCS检验和               

路由器和交换机：csma/cd协议，帧首部：源mac和目标mac ，MTU:1500字节          
路由器和路由器：ppp 点对点协议          
...
源IP地址、目标ip地址       
标识、标志、片偏移。标志为有3个，一个保留位，一个DF是否允许分片，一个MF是否是最后一片          
