...
-(NSDictionary<id,LCPathInfo*>*)__dijkstra:(id)begin{
    ...
    while (paths.count) {
        ...
        [self __relaxWithSelectedPaths:selectedPaths paths:paths lastEntry:minEntry];
    }
...
/// 松弛操作
/// @param selectedPaths 已经被选中的 Value:LCPathInfo
/// @param paths 待选的LCVertex:LCPathInfo
/// @param lastEntry 上次被选中的pair<LCVertex*, LCPathInfo*>
-(void)__relaxWithSelectedPaths:(NSMutableDictionary<id,LCPathInfo*>*)selectedPaths
                          paths:(NSMutableDictionary<LCVertex*,LCPathInfo*>*)paths
                      lastEntry:(pair<LCVertex*, LCPathInfo*>)lastEntry{
    
    LCVertex    * minVertex = lastEntry.first;
    LCPathInfo  * minPathInfo  = lastEntry.second;
    id minWeight = minPathInfo.weight;
    ...
    for (LCEdge* edge in minVertex.outEdges) {
        ...
        id newWeight = [self.weightManager addE1:minWeight e2:edge.weight];

        // 以前的最短路径：beginVertex到edge.to的最短路径
        LCPathInfo* oldPathInfo = [paths objectForKey:edge.to];
        if (oldPathInfo == nil) {
            
             //更新weighgt
            oldPathInfo =  [[LCPathInfo alloc] initWithWeight:newWeight];
            [paths setObject:oldPathInfo forKey:edge.to];
            
        }else if([self.weightManager compareE1:newWeight e2:oldPathInfo.weight]<0){
             //更新weighgt
             oldPathInfo.weight = newWeight;
             [oldPathInfo.edgeInfos removeAllObjects];
        }
        
        //更新edgeinfos
        LCPathInfo* pathInfo = [selectedPaths objectForKey:edge.from.value];
        [oldPathInfo.edgeInfos addObjectsFromArray:pathInfo.edgeInfos];
        [oldPathInfo.edgeInfos addObject:edge.info];
    }
}
```
