...
<!-- ************************************************ -->
## <a id="content3">3 Interview For Runloop</a>
...
<!-- ************************************************ -->
## <a id="content4">4 Interview For Thread</a>
...
<!-- ************************************************ -->
## <a id="content5">5 Interview For Memory</a>
...
6、autorelease对象在什么时机会被调用release
	...
	自动释放池的创建分两种
		手动创建自动释放池
			放入该释放池的autorelease对象会在离开释放池的作用域时被释放

		系统创建自动释放池
			runloop负责管理自动释放池的创建和销毁。
...
7、方法里有局部对象， 出了方法后会立即释放吗
	...
	__weak修饰的对象也会被注册到自动释放池
	
```


<!-- ************************************************ -->
## <a id="content6">6 内存优化</a>
